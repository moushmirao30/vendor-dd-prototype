...
| **Development & Deployment** | Python, Streamlit, `requests`, BeautifulSoup, `trafilatura`, pandas. JSON as the canonical store. 94 offline tests. One command to run: `streamlit run app.py`. |
...
config/vendors.yaml          config/field_dictionary.yaml     config/settings.yaml
  7 vendors, 43 seed URLs      8 fields, 112 phrases            all policy: UA, delays,
  url_patterns                 preferred_source_types           caps, thresholds
        │                              │                                │
        ▼                              ▼                                ▼
┌───────────────────┐        ┌────────────────────┐        ┌────────────────────┐
...
│                   │ Source │                    │Extract-│                    │
│ robots.txt → HTTP │ Records│ raw HTML → blocks  │edFields│ score, flag gaps,  │
│ → cache → text    │        │ → term match →     │        │ assemble brief     │
│                   │        │ rank → confidence  │        │   (NOT BUILT YET)  │
└───────────────────┘        └────────────────────┘        └────────────────────┘
        │                              │                                │
        ▼                              ▼                                ▼
 data/cache/html/            data/corpus/                     data/briefs/
 v2_<hash>.html              <slug>.json         (canonical)  <slug>_brief.json
 (offline replay)            <slug>_run.json     (trail)      + CSV + Markdown
                             <slug>_fields.json  (+ trail)
```
...
people browsing the corpus. On GitLab's security page `trafilatura` kept **7.6%** of the visible
text — 487 characters out of 6,415 — and what it discarded included every certification sentence.
Had extraction been built on that field, this system would have reported "no certifications found"
for a vendor that publishes them in plain prose under an `<h3>SOC Certification</h3>` heading.
...
image's `alt` attribute. Measured on the GitLab corpus: **8 of 92 evidence blocks were orphan
citations; now 0**, guarded by a property test over every fixture rather than by two examples.
...
## 6. Agent 3 — Brief Review · **not yet built**

Planned scope, stated here so the boundary is explicit rather than aspirational:

- Apply the vendor-level score in `docs/confidence_rules.md` to the five core fields.
- List missing or unclear fields explicitly, so a brief says "no privacy page found" rather than
  silently omitting the row.
- Raise review flags. Two are already known to be needed from real data:
  - **evidence from outside a field's natural home.** On GitLab, integrations/API and
    ...
    page is both authoritative and full of complete sentences. The match is real; the finding is
    weak; a human must see that.
  - **evidence that is gated.** Sentry publishes that its SOC 2 reports are "available to customers
    … upon request" — a genuine claim whose proof we cannot read.
- Assemble `VendorBrief`, then `src/export.py` (JSON / CSV / Markdown) and `src/orchestrator.py`
  (runs 1 → 2 → 3 in sequence).
...
yields 138 characters from 1.2 MB. Both are rendered in the browser. Atlassian's security, privacy,
...
- It measures how well-evidenced a statement is, **not whether the statement is true, and not
  whether it is about the thing you asked.** GitHub's sentence "GitHub's API stays secure with ISO,
  SOC 2, and GDPR" is a full sentence on an authoritative page, so the rule rates it High — but it
  is a claim about the API, not the platform. No rule-based system reads scope. This is a
  human-review case by design, which is why the quote and its URL are printed beside every label.
